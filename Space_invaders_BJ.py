...
def parse_arguments():
    ...
    parser.add_argument('--buffer_size', type=int, default=500000, help='Replay memory buffer size')
    ...
    parser.add_argument('--memory_burn_limit', type=int,default=250000, help='Till when to burn memory')
...
#Updates
#0) Based on Space_invaders_backup_code_change.py
#1) Changed the image normalization function
#2) Using state.tolist() while storing in memory in burn_memory()
#3) FloatTensor(next_state) instead of FloatTensor([next_state]).........didn't do this instead used np.float64 everywhere
