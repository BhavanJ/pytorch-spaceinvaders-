...
class Agent(object):
    def __init__(self, args, render=False):
        ...
        if args.model_type == 'Space_Invaders_CNN':
            ...
            self.target = Space_Invaders_CNN()
...
################################################################################################################################################
        ...
        while True:
            ...
            if is_terminal:
                if train:
                    ...
                    self.optimize_model()
                    #if steps %20 == 0:
                    print("Episode {} completed after {} steps till now | Total reward = {}".format(e+1,self.steps_done,total_reward))
    ...
    def optimize_model(self):
        ...
        # isolate the values
        non_terminal_mask = torch.from_numpy(np.array(list(map(lambda s: s is not None, batch.next_state))).astype(np.int64)).type(ByteTensor)
        ...
        # pdb.set_trace()
        next_state_values[non_terminal_mask] = self.target(batch_next_state).detach().max(1)[0]

        expected_Q = (self.gamma * next_state_values) + batch_reward
        expected_Q.volatile = False
...
def parse_arguments():
    ...
    parser.add_argument('--name',type=str, default='sisi', help='name of the expt - also used as folder name for saving outputs')
    ...
    parser.add_argument('--target_update', type=int, default=10, help='target network update frequency in number of steps')
    parser.add_argument('--batch_size', type=int, default=32, help='batch size')
    parser.add_argument('--buffer_size', type=int, default=5000, help='Replay memory buffer size')
    ...
    parser.add_argument('--eps_end', type=float, default=0.1, help='e-greedy threshold end value')
    parser.add_argument('--eps_decay', type=int, default=500000, help='e-greedy threshold decay')
    parser.add_argument('--logs', type=str, default = 'logs',  help='logs path')
    parser.add_argument('--memory_burn_limit', type=int,default=2500, help='Till when to burn memory')
